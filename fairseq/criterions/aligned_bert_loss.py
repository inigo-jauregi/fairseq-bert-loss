...
def aligned_bert_loss(lprobs, target, epsilon, ignore_index=None, reduce=True):
...
class AlignedBertLossCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss, n_correct, total_n = self.compute_loss(model, net_output, sample, reduce=reduce)
        # print(loss)
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        print_f_bert = (f_bert_avg).detach().cpu().numpy()
        self.loss_stats_file.write(str(print_acc) + '\t' + str(print_f_bert) + '\n')
